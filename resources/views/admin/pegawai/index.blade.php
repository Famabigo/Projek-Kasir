...
<div class="py-8" style="background-color: #EDF7FC; min-height: 100vh;">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.pegawai.create') }}" class="text-white px-4 py-2 rounded-lg font-medium transition-opacity hover:opacity-90 flex items-center space-x-2" style="background-color: #0C5587;">
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-3">
                                <span class="font-semibold text-sm" style="color: #0C5587;">#{{ $p->id }}</span>
                            </td>
                            <td class="px-6 py-3">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <span class="font-medium text-sm text-gray-900">{{ $p->nama }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $p->user->email ?? '-' }}</td>
                            <td class="px-6 py-3">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('admin.pegawai.edit',$p) }}" class="p-2 rounded-lg transition-colors hover:bg-blue-50" title="Edit">
                                        <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.pegawai.destroy',$p) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus pegawai ini?')" class="inline">
                                        @csrf @method('DELETE')
