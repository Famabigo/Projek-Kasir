...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6" style="background-color: #EDF7FC; min-height: 100vh;">
    ...
    <div class="mb-6">
        <div class="flex items-center space-x-3">
            <a href="{{ route('pembeli.cart') }}" class="p-2 rounded-lg bg-white hover:bg-gray-50 transition-colors">
    ...
    <form action="{{ route('pembeli.checkout.store') }}" method="POST" class="space-y-4">
        @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div id="delivery_form" class="hidden space-y-4">
                            <div>
                                ...
                                <textarea name="alamat_pengiriman" 
                                    ...
                                    class="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#0C5587] focus:border-transparent resize-none">{{ old('alamat_pengiriman', Auth::user()->alamat) }}</textarea>
                            </div>
                        ...
                        <div>
                            ...
                            <textarea name="catatan" 
                                ...
                                class="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#0C5587] focus:border-transparent resize-none @error('catatan') border-red-500 @enderror">{{ old('catatan') }}</textarea>
                            @error('catatan')
                        ...
                        @foreach($cart as $id => $item)
                            <div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-medium text-gray-900 truncate text-sm">{{ $item['name'] }}</h3>
                                    @if(isset($item['ukuran']) && $item['ukuran'])
                                        <p class="text-xs text-gray-500">{{ $item['ukuran'] }}</p>
                                    @endif
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <div class="flex justify-between items-center p-2 rounded-lg bg-green-50 border border-green-200">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-sm font-medium text-green-700">Diskon Member {{ number_format($diskonPersen, 0) }}%</span>
                            </div>
                            <span class="text-sm font-bold text-green-700">- Rp {{ number_format($diskonNominal, 0, ',', '.') }}</span>
                        </div>
                        @elseif(Auth::user()->member_status === 'approved' && $total < 50000)
                        <div class="flex items-start gap-2 p-2 rounded-lg bg-blue-50 border border-blue-200">
                            ...
                            <span class="text-xs text-blue-700">Belanja Rp {{ number_format(50000 - $total, 0, ',', '.') }} lagi untuk diskon 15%</span>
                        </div>
                        @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const baseTotal = {{ $totalSetelahDiskon }};
