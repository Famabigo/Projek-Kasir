...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="card-hover bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-6">
            <form method="GET" action="{{ route('kasir.stok.index') }}">
                <div class="flex gap-3">
                    <div class="flex-1 relative">
                        ...
                        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama barang atau kategori..." class="w-full pl-10 pr-4 py-3 rounded-lg border-gray-300 transition-all" style="border-color: #B1D7F2;" onfocus="this.style.borderColor='#0C5587'; this.style.outline='2px solid #EDF7FC'" onblur="this.style.borderColor='#B1D7F2'; this.style.outline='none'">
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($barang as $b)
                        <tr class="hover:bg-blue-50 transition-colors {{ $b->expired_at && $b->expired_at->isPast() ? 'bg-red-50' : '' }}">
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3">
                                    @if($b->gambar)
                                    <img src="{{ Storage::url($b->gambar) }}" alt="{{ $b->nama_barang }}" class="h-12 w-12 rounded-lg object-cover border-2" style="border-color: #B1D7F2;">
                                    @else
                                    ...
                                    <div>
                                        ...
                                        @if($b->expired_at && $b->expired_at->isPast())
                                        <span class="inline-block bg-red-100 text-red-800 text-xs font-bold px-2 py-0.5 rounded mt-1">EXPIRED</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $b->kategori ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $b->ukuran ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <span class="font-semibold text-green-600">Rp {{ number_format($b->harga_jual, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($b->stok < 10)
                                <span class="bg-red-100 text-red-800 px-3 py-1 rounded-full text-sm font-semibold">{{ $b->stok }}</span>
                                @else
                                <span class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-semibold">{{ $b->stok }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm">
                                @if($b->expired_at)
                                    @if($b->expired_at->isPast())
                                    <span class="text-red-600 font-semibold">{{ $b->expired_at->format('d M Y') }}</span>
                                    @elseif($b->expired_at->diffInDays() < 90)
                                    <span class="text-orange-600 font-semibold">{{ $b->expired_at->format('d M Y') }}</span>
                                    <div class="text-xs text-orange-600 mt-1">({{ $b->expired_at->diffInDays() }} hari lagi)</div>
                                    @else
                                    <span class="text-gray-700">{{ $b->expired_at->format('d M Y') }}</span>
                                    @endif
                                @else
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @if($perluLapor)
                                    ...
                                    @else
                                        <button onclick="openLaporModal({{ $b->id }}, '{{ addslashes($b->nama_barang) }}', '{{ $tipeLapor }}')" 
