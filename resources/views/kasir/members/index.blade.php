...
<div class="py-8" style="background-color: #EDF7FC; min-height: 100vh;">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('kasir.members.create') }}" class="text-white px-4 py-2 rounded-lg font-medium transition-opacity hover:opacity-90 flex items-center space-x-2" style="background-color: #0C5587;">
        ...
        <div class="mb-6">
            <div class="flex space-x-2 border-b-2 border-gray-200">
                ...
                <button onclick="showTab('pending')" id="tab-pending" class="tab-btn px-6 py-3 font-semibold text-gray-500 hover:text-gray-700 transition-all duration-200 border-bottom-3 border-transparent">
                    ...
                    @endphp
                    @if($pendingCount > 0)
                        <span class="ml-2 px-2 py-1 text-xs font-bold text-white rounded-full" style="background: #C7E339; color: #0C5587;">{{ $pendingCount }}</span>
                    @endif
                </button>
            </div>
        </div>
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-3">
                                <span class="font-semibold text-sm" style="color: #0C5587;">#{{ $user->id }}</span>
                            </td>
                            <td class="px-6 py-3">
                            ...
                            <td class="px-6 py-3">
                                <span class="px-2.5 py-1 rounded-full text-xs font-semibold text-white" style="background: linear-gradient(135deg, #0C5587 0%, #0884D1 100%);">{{ $user->kode_member }}</span>
                            </td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $user->email }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $user->no_hp ?? '-' }}</td>
                        </tr>
                        @empty
                        <tr>
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-3 text-sm text-gray-700">{{ $user->email }}</td>
                                <td class="px-6 py-3 text-sm text-gray-700">{{ $user->no_hp ?? '-' }}</td>
                                <td class="px-6 py-3 text-sm text-gray-700">{{ $user->alamat ?? '-' }}</td>
                                <td class="px-6 py-3">
                                    <div class="flex items-center justify-center space-x-2">
                                        <button onclick="openApproveModal({{ $user->id }}, '{{ $user->name }}')" class="p-2 rounded-lg transition-colors hover:bg-green-50" title="Setujui">
                                            <svg class="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                            </svg>
                                        </button>
                                        <button onclick="openRejectModal({{ $user->id }}, '{{ $user->name }}')" class="p-2 rounded-lg transition-colors hover:bg-red-50" title="Tolak">
