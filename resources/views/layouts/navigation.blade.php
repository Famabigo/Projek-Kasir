            ...
            <div class="flex justify-center gap-8">
                ...
                <a href="{{ route('kasir.transaksi.index') }}" class="flex items-center space-x-2 text-gray-700 hover:text-[#0C5587] pb-2 {{ request()->routeIs('kasir.transaksi.index') || request()->routeIs('kasir.transaksi.show') ? 'border-b-2 border-[#0C5587] text-[#0C5587]' : '' }}">
                    <div class="relative">
                        ...
                        @endphp
                        @if($pesananMenunggu > 0)
                        <span class="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center animate-pulse">{{ $pesananMenunggu }}</span>
                        @endif
                    </div>
                    <span>Transaksi</span>
                </a>
                <a href="{{ route('kasir.laporan.index') }}" class="flex items-center space-x-2 text-gray-700 hover:text-[#0C5587] pb-2 {{ request()->routeIs('kasir.laporan.*') ? 'border-b-2 border-[#0C5587] text-[#0C5587]' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
